...
def kmeans_k_diawal(X, k, metode, max_iter=100):
    ...
    for iterasi in range(1, max_iter + 1):
        ...
        label_baru = np.argmin(semua_jarak, axis=1)


        centroid_baru = []
        for i in range(k):
            data_cluster = X[label_baru == i]
        ...
        if np.array_equal(label_baru, label_lama):
            break

        centroid = centroid_baru
        label_lama = label_baru

    return label_baru, centroid, iterasi
...
for idx, metode in enumerate(metode_jarak, start=1):
    label, centroid, iterasi = kmeans_k_diawal(X, k, metode)
    nama_cluster = beri_nama_cluster(label)

    print("\n==============================")
    print("K-MEANS DENGAN K DI AWAL")
...
plt.suptitle("K-Means dengan K di Awal - Dataset RT-IoT2022 Asli")
plt.tight_layout()
plt.savefig("grafik_kmeans_k_diawal.png", dpi=200)
plt.show()
